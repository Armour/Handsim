...
def I_type(line):
    if I_dict[line[0:6]][0] == 0:
        ...
        return I_dict[line[0:6]][1] + ' ' + code[line[6:11]] + ', ' + 'mark_' + str(int(line[16:], 2)/4)
    elif I_dict[line[0:6]][0] == 2:
        return I_dict[line[0:6]][1] + ' ' + code[line[6:11]] + ', ' + code[line[11:16]] + ', mark_' + str(int(line[16:], 2)/4)
...
def J_type(line):
    return J_dict[line[0:6]] + ' mark_' + str(int(line[6:], 2)/4)
...
def print_command():
    with open('back.txt', 'w+') as output:
        for line in command:
            output.write(line + '\n')
